...
chroot $TEMPMOUNT /bin/bash -c "apt-get install -y mbpfan && echo '---> apt install mbpfan succeeded <--------------------------------------------------------------' || { echo 'apt install mbpfan failed'; exit 1; }" || exit 1

if [[ -n "$MACWIFIWL" ]]
then
    chroot $TEMPMOUNT /bin/bash -c "apt-get install -y broadcom-sta-common && echo '---> apt install broadcom-sta-common succeeded <--------------------------------------------------------------' || { echo 'apt install broadcom-sta-common failed'; exit 1; }" || exit 1

elif [[ -n "$MACWIFIB43" ]]
then

    chroot $TEMPMOUNT /bin/bash -c "apt-get install -y firmware-b43-installer && echo '---> apt install firmware-b43-installer succeeded <--------------------------------------------------------------' || { echo 'apt install firmware-b43-installer failed'; exit 1; }" || exit 1
