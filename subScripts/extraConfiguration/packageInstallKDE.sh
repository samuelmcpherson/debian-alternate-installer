...
chroot $TEMPMOUNT /bin/bash -c "apt install -y kde-plasma-desktop network-manager plasma-nm && echo '---> apt install kde-plasma-desktop succeeded <--------------------------------------------------------------' || { echo 'apt install kde-plasma-desktop failed'; exit 1; }" || exit 1
